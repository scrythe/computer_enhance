...
const file_name = "listing_0057_challenge_cycles";
...
fn execute_instructions(
    ...
    while (ip_reg.* < listing_content.len) {
        ...
        for (decoded.Operands, 0..) |operand, i| {
            switch (operand.Type) {
                ...
                .OperandMemory => {
                    ...
                    uneven_addr = operands_effec_addr_data[i] % 2 == 1;
                    const low = memory[operands_effec_addr_data[i]];
                    const high: u16 = if (decoded.Flags.Wide) memory[operands_effec_addr_data[i] + 1] else 0;
        ...
        var cycles_penalty: u32 = 0;
        if (uneven_addr) {
            cycles_penalty = 4 * transfers;
        } else if (!is_8086) {
            cycles_penalty += 4 * transfers;
        }
        ...
        if (effect_addr_clock != 0) {
            ...
            if (cycles_penalty != 0) {
                try temp_print_writer.print(" + {d}p", .{cycles_penalty});
            }
            try temp_print_writer.print(")", .{});
            clocks_explanation = temp_print_buffer[start..temp_print_writer.end];
        }

        clocks_addition += effect_addr_clock + cycles_penalty;
